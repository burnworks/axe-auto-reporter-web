...
import config, { runtimeSettings } from './config.mjs';
...
const parseRunTimestamp = (runId) => {
    ...
    const adjusted = new Date(date.getTime() - JST_OFFSET_MINUTES * 60 * 1000);
...
const saveReportsIndex = async (index) => {
    await fs.mkdir(REPORTS_DIR, { recursive: true });
    await fs.writeFile(REPORTS_INDEX_PATH, JSON.stringify(index, null, 2), 'utf-8');
};
...
const buildSummary = async () => {
    ...
    const jsonDir = path.join(normalizedBasePath, '_json');
    const runId = path.basename(normalizedBasePath);
    const runTimestamp = parseRunTimestamp(runId) ?? new Date().toISOString();
    ...
    for (const file of jsonFiles) {
        ...
        let baseFilename = (data && typeof data === 'object' && data.metadata && typeof data.metadata.baseFilename === 'string')
            ? data.metadata.baseFilename
            : null;
        ...
        <tr data-minor="${impactCounts.minor}" data-moderate="${impactCounts.moderate}" data-serious="${impactCounts.serious}" data-critical="${impactCounts.critical}" data-total="${totalIssues}">
            <th scope="row">
                <div class="report-link">
                    <a href="/results/${escapeHtml(runId)}/${escapeHtml(baseFilename)}" title="${translate('linkToDetailReport').replace('{url}', escapeHtml(url))}" aria-label="${translate('linkToDetailReport').replace('{url}', escapeHtml(url))}">${escapeHtml(url)}</a>
    ...
    const stylePath = path.join(process.cwd(), 'src', 'styles', 'global.css');
    let styleContent = '';
    try {
        styleContent = await fs.readFile(stylePath, 'utf-8');
    } catch (error) {
        console.warn('Warning: Could not load global.css for summary.', error);
        styleContent = `/* Summary styles unavailable */`;
    }
    ...
    const resultsRelativePath = path.relative(process.cwd(), normalizedBasePath);

    const summaryData = {
        runId,
        runTimestamp,
        generatedAt,
        resultsDir: resultsRelativePath,
        totalPages,
        globalTotal,
        totals: { ...globalStats },
        occurrenceRates: { ...issueOccurrenceRates },
        pages: pageStats,
        settings: {
            sitemapUrl: runtimeSettings.sitemapUrl,
            tags: runtimeSettings.tags,
            mode: runtimeSettings.mode,
            maxPages: runtimeSettings.maxPages,
            frequency: runtimeSettings.frequency ?? 'daily'
        }
    };

    const summaryDir = path.join(normalizedBasePath, '_summary');
    ...
    const summaryJsonPath = path.join(normalizedSummaryDir, '_summary.json');
    try {
        await fs.writeFile(summaryJsonPath, JSON.stringify(summaryData, null, 2), 'utf-8');
        console.log(`Summary JSON generated at ${summaryJsonPath}`);
    } catch (error) {
        console.error(`Error: Failed to write summary JSON ${summaryJsonPath}.`, error);
        process.exit(1);
    }
    ...
    const indexEntry = {
        ...
        totalPages,
        globalTotal,
        tags: summaryData.settings.tags,
        mode: summaryData.settings.mode,
        maxPages: summaryData.settings.maxPages,
        frequency: summaryData.settings.frequency
    };

    const index = await loadReportsIndex();
    index.runs = Array.isArray(index.runs) ? index.runs : [];
    index.runs = index.runs.filter((run) => run.runId !== runId);
    index.runs.push(indexEntry);
    index.runs.sort((a, b) => {
        const aTime = new Date(a.runTimestamp || a.generatedAt || 0).getTime();
        const bTime = new Date(b.runTimestamp || b.generatedAt || 0).getTime();
        return bTime - aTime;
    });

    await saveReportsIndex(index);
}
